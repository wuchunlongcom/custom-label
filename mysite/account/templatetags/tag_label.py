...
# 自定义标签   功能：当前时间转换为字符串 get_current_time 没有参数
@register.simple_tag()
def get_current_time(format_string):
    return datetime.now().strftime(format_string) # 当前时间转换为字符串

# 自定义标签 功能：获得数据库记录 pomes_of_author 一个参数：作者姓名
@register.inclusion_tag("account/resultes.html")
def pomes_of_author(author_name):
    poems = Poem.objects.filter(author=author_name)
    return {"pomes": poems, "author_name": author_name} # 数据库记录作者、作者姓名
